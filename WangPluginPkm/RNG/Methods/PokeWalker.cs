...
namespace WangPluginPkm.RNG.Methods
{
    internal class PokeWalker
    {
        ...
        public static bool GenPkm(ref PKM pk, CheckRules r)
        {
            ...
            do
            {
                pk.PID = GetPokeWalkerPID(pk.TID, pk.SID, pk.Nature, pk.Gender, pk.PersonalInfo.Gender);
            } while (!pk.IsGenderValid());

            pk.RefreshAbility((int)(pk.PID & 1));
            return true;
        }
        public static uint GetPokeWalkerPID(int TID, int SID, int nature, int gender, int gr)
        {
            if (nature >= 24)
                nature = 0;
