...
public static class LumioseRNG
{
        ...
        byte? desiredGender = null)
    {
        ...
        // 2) 构造 criteria：尽量“放宽”，避免生成阶段被提前否掉
        var criteria = new EncounterCriteria();

        // 3) 生成：用临时个体承接，避免失败时污染外部 pk
        var cand = new PA9
        {
            Species = pk.Species,
            Form = pk.Form,

            // 这些ID会影响 shiny 适配与 PID 调整，务必从原 pk 传入
            TID16 = pk.TID16,
            SID16 = pk.SID16,
            ID32 = pk.ID32,
        };

        bool ok = LumioseRNG.GenerateData(cand, enc, criteria, seed); // 失败直接丢弃该 seed
        if (!ok)
            return false;

        // 4) 命中后按你的规则二次校验（IV 区间、更多条件等）
        if (!CheckIV(r, cand))
            return false;
