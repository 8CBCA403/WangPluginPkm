...
#nullable enable
        public static ITeraRaid? GetEncounter(uint Seed, int stage, bool black)
        {
            ...
            var starcount = black ? 6 : TeraRaid.GetStarCount((uint)clone.NextInt(100), stage, false);
            var total = TeraRaid.Game == "Scarlet" ? GetRateTotalBaseSL(starcount) : GetRateTotalBaseVL(starcount);
            var speciesroll = clone.NextInt((ulong)total);
            if (TeraRaid.GemTeraRaids != null)
            {
                foreach (TeraEncounter enc in (TeraEncounter[])TeraRaid.GemTeraRaids)
                {
                    if (enc.Stars != starcount)
                        continue;
                    var minimum = TeraRaid.Game == "Scarlet" ? enc.Entity.RandRateMinScarlet : enc.Entity.RandRateMinViolet;
